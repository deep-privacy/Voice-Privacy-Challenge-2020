...
if [ $stage -le 1 ]; then
  python pchampio/local/featex/create_bn_data.py pchampio/exp/${data_dir}_hires/dump_eproj ${out_dir} || exit 1;
fi

if [ $stage -le 2 ]; then
  echo "Writing xvector and F0 for train."
  python pchampio/local/featex/create_xvector_f0_data.py ${src_data} ${xvector_file} ${out_dir} || exit 1;
fi
